...
class CustomerConnectionHandler extends ChatConnectionHandler {
  constructor (socket, messageRouter, onDisconnect) {
    ...
    // Here, we use the socket's unique id as a customer id.
    this.init(socket.id);
    this.attachHandlers();
  }
  ...
  _respondToCustomer (response) {
    ...
    // Socket.io is used for our chat, which provides a synchronous API. However, in later case
    // can swich it out for an async call, this method returns a promise.
